...
- **2026-09-24 · Release config (#20).** Portrait only on iPhone and Android
  ...
  tracking, no data collected, UserDefaults reason CA92.1); R8 on Android;
  release signing from `android/key.properties`, debug key when absent
  (docs/RELEASING.md). About screen (Settings → About) with version, website,
  privacy, terms, contact and licenses; `url_launcher` added to open them.
  The version shown is a constant in `lib/data/app_info.dart` checked against
  pubspec by a test, instead of another plugin.

- **2026-09-24 · Motion (#18).** Durations and easing come from `AppMotion`.
  Every non-essential animation (Hero of the exercise icon, 1RM counter,
  staggered lists, Android page transitions) is static when the system asks
  for reduced motion. iOS keeps its page slide even then: the swipe-back
  gesture lives in it and must always work.
